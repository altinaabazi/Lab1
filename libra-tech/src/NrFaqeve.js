...
export class NrFaqeve extends Component{
    ...
    editClick(dep){
        this.setState({
            modalTitle:"Ndrysho Nr.Faqeve",
            ID:dep.ID,
            nrfaqeve:dep.nrfaqeve
        });
    }

    createClick() {
        const { nrfaqeve } = this.state;
    
        if (!nrfaqeve) {
            alert('Ju lutem vendosni Nr.Faqeve.');
            return;
        }
    ...
    deleteClick(id){
        ...
        fetch(variables.API_URL+'NrFaqeve/'+id,{
            method:'DELETE',
            headers:{
                'Accept':'application/json',
                'Content-Type':'application/json'
